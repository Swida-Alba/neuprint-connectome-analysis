...
*   `{ABBREV}` — dataset abbreviation used in folder names: `MCNS` (male-cns), `FAFB` (FAFB local release), `HEMI` (hemibrain), `MANC` (manc), `OLOB` (optic-lobe). Standalone BANC appears as `B_v626` / `B_v888`.
